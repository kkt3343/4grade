 <?php 
	...
	$fail = array(
		...
		array(100, 0, 0, 0, 0, 0, 0, 0, 0, 0),
		array(65, 35, 0, 0, 0, 0, 0, 0, 0, 0),
		array(55, 45, 0, 0, 0, 0, 0, 0, 0, 0),
		array(35, 40, 25, 0, 0, 0, 0, 0, 0, 0),
		array(10, 32, 36, 22, 0, 0, 0, 0, 0, 0),
		array(4, 10, 30, 35, 21, 0, 0, 0, 0, 0),
		array(2, 4, 10, 28, 35, 21, 0, 0, 0, 0),
		array(1, 2, 4, 10, 28, 34, 21, 0, 0, 0)
	);
	...
	//강화비용 계산
	$success_price = array(
			intval($my_reinforce_item['item_original_price'])*0.05,
			intval($my_reinforce_item['item_original_price'])*1.2 * 0.05,
			intval($my_reinforce_item['item_original_price'])*2 * 0.05,
			intval($my_reinforce_item['item_original_price'])*3 * 0.05,
			intval($my_reinforce_item['item_original_price'])*4 * 0.05,
			intval($my_reinforce_item['item_original_price'])*5 * 0.05,
			intval($my_reinforce_item['item_original_price'])*6 * 0.05,
			intval($my_reinforce_item['item_original_price'])*8 * 0.05,
			intval($my_reinforce_item['item_original_price'])*10 * 0.05
	);
	...
	if(intval($item_reinforce) >= 10){
		...
		echo "<form name=\"fail\" action=\"inventory_reinforce.php\" method=\"post\">
			...
			<input type=\"hidden\" name=\"success_or_fail\" value= \"-\">
