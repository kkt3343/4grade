...
<body>
	...
	<?php 
		...
		$success_price = array(
			intval($row['item_original_price'])*0.05,
			intval($row['item_original_price'])*1.2 * 0.05,
			intval($row['item_original_price'])*2 * 0.05,
			intval($row['item_original_price'])*3 * 0.05,
			intval($row['item_original_price'])*4 * 0.05,
			intval($row['item_original_price'])*5 * 0.05,
			intval($row['item_original_price'])*6 * 0.05,
			intval($row['item_original_price'])*8 * 0.05,
			intval($row['item_original_price'])*10 * 0.05
			);
	?>
	<table>
		<thead>
