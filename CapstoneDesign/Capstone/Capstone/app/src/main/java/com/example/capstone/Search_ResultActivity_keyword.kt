...
class Search_ResultActivity_keyword : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, saveInstanceState: Bundle?): View? {
        ...
        var layout = inflater.inflate(R.layout.search_result_keyword, container, false)
        ...
        //몇대 존재합니다.
        var car_count = layout.findViewById<TextView>(R.id.search_result_keyword_show_car_count)
        ...
        view_number_spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                if(position==0){
                    page_number = 1
                    view_number = 5
                    spinner_accept(layout, context, keyword, car_count)
                }
                else if(position==1){
                    page_number = 1
                    view_number = 10
    ...
    fun item_load(keyword: String){
        //쓰레드를 하고 마칠 때 까지 기다린다.
        var thread = NetworkThread_keyword()
...
//        }
    ...
    fun layout_load(context: Context, layout : View, unitnumber : ExchangeFormat, unitnumber2 : ExchangeFormat_1000){
        ...
        for (i in 0 until getcount)
        {
            ...
            if(coef[i] == "" || intercept[i] == ""){
                ...
                var priper = 0f
                priper = Math.round(((prigap / (expected_price).toFloat()) * 100) * 100) /100.toFloat()
                var showprigap =  unitnumber.unit_change(prigap.toString())

                if(expected_price < price[i].toInt()){
                    Price_string2 = "(+" + showprigap + "원) (" + Math.abs(priper) + "% 높음)\n"
                    text4 += Price_string2
                    ...
                    Price_string2 = "(" + showprigap + "원) (" + Math.abs(priper) + "% 낮음)\n"
                    text4 += Price_string2
    ...
    //서버통신
    inner class NetworkThread_keyword : Thread() {

        var keyword = ""

        override fun run() {
            // 접속할 페이지 주소: Site
            var site = "http://"+ netip +"/car_keyword_search.php?keyword="+keyword+"&page_number="+page_number+"&view_number="+view_number+"&order_type="+ordertype
            Log.d("테스트site",site)
    ...
    // n개씩 보기
    fun spinner_accept(layout: View, context: Context, keyword : String, car_count : TextView){
        //쓰레드를 하고 마칠 때 까지 기다린다.
        item_load(keyword)
        // ----------------------------------------//
        //거리 & 가격 변수
        var unitnumber = ExchangeFormat();
        //var avg_price_show = unitnumber.unit_change(avgpriceINT.toString())
        var unitnumber2 = ExchangeFormat_1000();
//        //var avg_distance_show = unitnumber2.unit_change(avgdistanceINT.toString())
//        //가격
//        avg_use1.setSingleLine(true); //한줄로 나오게 하기.
//        avg_use1.setEllipsize(TextUtils.TruncateAt.MARQUEE);//Ellipsize의 MARQUEE 속성 주기
//        avg_use1.setSelected(true); //해당 텍스트뷰에 포커스가 없어도 문자 흐르게 하기
//        if(avg_price_show==""){
//            avg_use1.text = "0원"
//        }else{
//            avg_use1.text = avg_price_show + "원"
//        }
//        //KM수
//        avg_use2.text = avg_distance_show + "KM"
//
...
//
        ...
        // 페이지 버튼 설정
        set_pageButton(layout, context, keyword , car_count)
    }
    fun set_pageButton(layout: View, context: Context, keyword: String, car_count : TextView) {
    ...
    override fun onHiddenChanged(hidden: Boolean) {
        ...
        //프래그먼트 하이드 상태일때 동작
        if(isHidden) {
            //mainactivity?.mMenu?.findItem(R.id.sort_menu)?.isVisible = false
        }
        else{
            //mainactivity?.mMenu?.findItem(R.id.sort_menu)?.isVisible = true
