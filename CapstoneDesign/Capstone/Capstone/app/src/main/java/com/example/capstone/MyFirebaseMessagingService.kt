...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(p0: RemoteMessage) {
        ...
        if(p0.data.isNotEmpty()){
            Log.i("바디: ", p0.data["body"].toString())
            Log.i("타이틀: ", p0.data["title"].toString())
            sendNotification(p0)
        }
    ...
    private fun sendNotification(remoteMessage: RemoteMessage) {
        ...
        //안드로이드 12 버전 이슈 FLAG_ONE_SHOT 을 FLAG_IMMUTABLE로 바꿈
        //참조 : https://todaycode.tistory.com/8
        val pendingIntent = PendingIntent.getActivity(this, uniId, intent, PendingIntent.FLAG_IMMUTABLE)

        // 알림 채널 이름
        val channelId = getString(R.string.fcm_message)
